...
/** Stored avatarAssetId forms: null (follow hero) · "<hero>-<variant>" · "companion:<id>" · photo URL / data-URL. */
...
/** True for any stored player photo (http URL or inlined data-URL). */
export function isPhotoAvatar(assetId?: string | null): boolean {
  return !!assetId && (assetId.startsWith("http://") || assetId.startsWith("https://") || assetId.startsWith("data:image/"));
}
...
export function resolveAvatar(avatarAssetId?: string | null, heroAssetIdFallback?: string | null): ResolvedAvatar {
  if (isPhotoAvatar(avatarAssetId)) {
    return { kind: "photo", hero: HEROES[0], variant: "traditional", companion: COMPANIONS[0], photoUrl: avatarAssetId as string };
  }
